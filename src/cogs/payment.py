...
from src.config.settings import get_welcome_message, ConfigConstants
...
class PaymentCog(commands.Cog):
    ...
    async def check_payment(self, ctx: commands.Context) -> None:
        ...
        async with get_db() as db:
            try:
                user = await self.get_user(db, ctx.author.id)
                if not user:
                    await ctx.send(f"{ctx.author.mention}, you do not have any tickets.")
                    return

                payment_intent_id = self.extract_payment_intent_id(ctx.message.content)
    ...
    async def get_user(self, db: AsyncSession, discord_id: int) -> Optional[User]:
        """
    ...
    def extract_payment_intent_id(self, message_content: str) -> Optional[str]:
        """
        ...
        payment_intent_id = next((word for word in words
                                  if word.startswith(ConfigConstants.PAYMENT_INTENT_PREFIX)), None)
        return payment_intent_id

    async def get_valid_attachment(self, ctx: commands.Context) -> Optional[discord.Attachment]:
        """
        ...
        if not attachment.filename.lower().endswith(ConfigConstants.VALID_ATTACHMENT_EXTENSIONS):
            await ctx.send(
    ...
    async def confirm_payment(
        ...
        """
        try:
            payment_verified = await self.verify_payment(payment_intent_id)
    ...
    async def verify_payment(self, payment_intent_id: str) -> bool:
        ...
        Returns:
            bool: True if payment is verified, else False.
        """
        loop = asyncio.get_event_loop()
        payment_verified = await loop.run_in_executor(None, verify_payment_intent, payment_intent_id)
    ...
    async def notify_admins(
        ...
        if not admin_channel:
            logger.error(f"Admin notification channel not found. Searched for channel ID: "
        ...
        try:
            ...
            logger.error(f"Error notifying admins in channel {ConfigConstants.ADMIN_CHANNEL_ID}: {e!s}", exc_info=True)

    async def get_admin_channel(self, current_guild: discord.Guild) -> Optional[discord.TextChannel]:
        """
        ...
        """
        admin_channel = current_guild.get_channel(ConfigConstants.ADMIN_CHANNEL_ID)
        if admin_channel:
